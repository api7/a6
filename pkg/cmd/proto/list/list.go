...
func listRun(opts *Options) error {
	...
	protos := make([]api.Proto, len(resp.List))
	...
	if format == "table" {
		if len(protos) == 0 {
			fmt.Fprintln(opts.IO.Out, "No proto definitions found.")
			return nil
		}
		tp := tableprinter.New(opts.IO.Out)
		tp.SetHeaders("ID", "NAME", "DESC", "CREATED")
		for _, p := range protos {
			created := ""
			if p.CreateTime != nil {
				created = time.Unix(*p.CreateTime, 0).Format("2006-01-02 15:04:05")
			}
			tp.AddRow(derefStr(p.ID), derefStr(p.Name), derefStr(p.Desc), created)
		}
		return tp.Render()
	}

	return cmdutil.NewExporter(format, opts.IO.Out).Write(protos)
}
