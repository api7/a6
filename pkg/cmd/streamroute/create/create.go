...
func NewCmdCreate(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a stream route from a file",
...
func createRun(opts *Options) error {
	...
	if id, ok := payload["id"]; ok {
		...
		body, err = client.Put(fmt.Sprintf("/apisix/admin/stream_routes/%s", idStr), payload)
	} else {
		body, err = client.Post("/apisix/admin/stream_routes", payload)
	}
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var resp api.SingleResponse[api.StreamRoute]
