...
func createRun(opts *Options) error {
	...
	if id, ok := payload["id"]; ok {
		...
		body, err = client.Put(fmt.Sprintf("/apisix/admin/routes/%s", idStr), payload)
	} else {
		body, err = client.Post("/apisix/admin/routes", payload)
	}
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var resp api.SingleResponse[api.Route]
