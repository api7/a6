...
func NewCmdGet(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "get <name>",
		Short: "Get plugin schema by name",
...
func getRun(opts *Options) error {
	...
	body, err := client.Get(fmt.Sprintf("/apisix/admin/plugins/%s", opts.Name), nil)
	if err != nil {
	...
	return cmdutil.NewExporter(format, opts.IO.Out).Write(schema)
}
