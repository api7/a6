...
func TestServiceUpdate_Success(t *testing.T) {
	...
	reg.Register(http.MethodPut, "/apisix/admin/services/1", httpmock.JSONResponse(updatedServiceBody))
	...
	err := os.WriteFile(filePath, []byte(`{"name":"updated-service","upstream":{"type":"roundrobin","nodes":{"127.0.0.1:8080":1}}}`), 0o644)
...
func TestServiceUpdate_NotFound(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodPut, "/apisix/admin/services/999", httpmock.StringResponse(404, `{"error_msg":"not found"}`))
...
func TestServiceUpdate_MissingArg(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestServiceUpdate_MissingFile(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestServiceUpdate_NoArgsNonTTY(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
